...
// Watermask raster tiles. `watermask` = Web Mercator XYZ (Protomaps native);
// `watermask-tms` = Cesium geodetic TMS, sharing coords with `cesium-mesh`.
const WATERMASK_WITH_TILESET = /^\/([a-z0-9-]+)\/(watermask|watermask-tms)\/(\d+)\/(\d+)\/(\d+)\.(webp|png)$/;
...
type WatermaskScheme = "watermask" | "watermask-tms";
...
async function handle(req: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
  ...
  try {
      ...
      // Watermask raster tiles.
      m = WATERMASK_WITH_TILESET.exec(url.pathname);
      if (m) {
        const tileset = resolveTileset(m[1], env);
        if (!tileset) return notFound(`unknown tileset: ${m[1]}`);
        return await serveWatermask(req, ctx, tileset, m[2] as WatermaskScheme, Number(m[3]), Number(m[4]), Number(m[5]), m[6] as Format, env);
      }
      m = WATERMASK_DEFAULT.exec(url.pathname);
      if (m) {
        const tileset = resolveTileset(undefined, env);
        if (!tileset) return notFound("default tileset not configured");
        return await serveWatermask(req, ctx, tileset, m[1] as WatermaskScheme, Number(m[2]), Number(m[3]), Number(m[4]), m[5] as Format, env);
      }

      m = WATERMASK_TILEJSON_WITH_TILESET.exec(url.pathname);
      if (m) {
        const tileset = resolveTileset(m[1], env);
        if (!tileset) return notFound(`unknown tileset: ${m[1]}`);
        return await watermaskTilejson(req, tileset, m[2] as WatermaskScheme);
      }
      m = WATERMASK_TILEJSON_DEFAULT.exec(url.pathname);
      if (m) {
        const tileset = resolveTileset(undefined, env);
        if (!tileset) return notFound("default tileset not configured");
...
async function serveWatermask(
  req: Request,
  ctx: ExecutionContext,
  tileset: Tileset,
  scheme: WatermaskScheme,
  z: number,
  x: number,
  y: number,
  format: Format,
  ...
  // XYZ uses Web Mercator (Protomaps' native projection); watermask-tms
  // uses the Cesium geodetic TMS bounds so the output aligns 1:1 with
  // /cesium-mesh tiles.
  const bounds =
    scheme === "watermask"
      ? tileToLonLatBounds(z, x, y)
      : geodeticTileBounds(z, x, y);

  const watermask = await resolveWatermask(tileset, bounds, env);
  if (!watermask) return notFound(`watermask disabled for tileset: ${tileset.name}`);
  ...
  return cachedTile(
    ...
    env.R2,
    {
      tileset: tileset.name,
      version: resolveTilesetVersion(tileset),
      encoding: `${scheme}-${watermask.version}`,
      dataType: "watermask",
      z,
      x,
      y,
    ...
    async () => {
      ...
      const bytes =
        format === "webp"
          ? rgba_to_webp(rgba, MASK_SIZE, MASK_SIZE)
          : rgba_to_png(rgba, MASK_SIZE, MASK_SIZE);
      return {
        bytes,
        contentType: format === "webp" ? "image/webp" : "image/png",
      };
...
async function watermaskTilejson(
  ...
  if (!tileset.watermask) return notFound(`watermask disabled for tileset: ${tileset.name}`);
  const base = new URL(req.url);
  base.pathname = `/${tileset.name}/${scheme}`;
  const tms = scheme === "watermask-tms";
  return metadataJson(req, {
    tilejson: "3.0.0",
    name: `${tileset.name} / ${scheme}`,
    description: tileset.description,
    version: resolveTilesetVersion(tileset),
    scheme: tms ? "tms" : "xyz",
    tiles: [`${base.toString()}/{z}/{x}/{y}.webp`],
    attribution: [...tileset.attribution, ...tileset.watermask.attribution]
      .map((a) => (a.url ? `<a href="${a.url}">${a.name}</a>` : a.name))
      .join(", "),
    minzoom: tileset.minZoom,
    maxzoom: tileset.maxZoom,
    bounds: tms ? [-180, -90, 180, 90] : [-180, -85.0511287798066, 180, 85.0511287798066],
  });
}
